  ...
  return (
    <div
      ...
      className={cn('group relative shrink-0 overflow-hidden rounded-xl text-left transition-all duration-200 ease-[var(--ease-out-quart)]',
        selected
          ? 'z-10 -translate-y-2 scale-[1.04] ring-2 ring-amber-300 shadow-[0_14px_40px_-10px_rgba(234,184,102,.5)]'
          : 'ring-1 ring-white/10 hover:z-10 hover:-translate-y-2 hover:scale-[1.06] hover:ring-white/30 hover:shadow-[var(--shadow-pop)]',
        marked && !selected && 'z-10 -translate-y-1 ring-2 ring-amber-400/70 shadow-[0_8px_28px_-10px_rgba(234,184,102,.5)]',
      ...
      style={{ width, aspectRatio: '16 / 9', contentVisibility: 'auto', containIntrinsicSize: `${width}px ${Math.round((width * 9) / 16)}px`, background: `linear-gradient(135deg, color-mix(in srgb, ${r.color} 22%, #0b0e14), #0b0e14)` }}
    >
      ...
      <span className="absolute left-2 top-2 flex h-6 items-center rounded-md bg-black/55 px-2 text-[12px] font-semibold tabular text-white backdrop-blur-sm">{c.scene}</span>
      <div className="absolute right-2 top-2 flex gap-1">
        {lowRes && <Tip label={`Düşük çözünürlük: ${c.resolution}p — 1080p değil`}><span className="flex h-6 items-center rounded-md bg-danger px-1.5 text-[10px] font-bold text-white shadow-[0_0_10px_var(--color-danger)]">{c.resolution}p</span></Tip>}
        ...
        {low && <Tip label={`Düşük güven · ${Math.round(c.decision.confidence * 100)}%`}><span className="flex h-6 w-6 items-center justify-center rounded-md bg-warn/90 text-[12px] font-bold text-ink-950">!</span></Tip>}
      </div>

      <div className="absolute bottom-2 left-2 flex items-center gap-1.5">
        <span className="flex items-center gap-1.5 rounded-md bg-black/50 px-2 py-1 text-[11.5px] tabular text-white/90 backdrop-blur-sm">
          <span className="h-2 w-2 rounded-full" style={{ background: r.color }} /> {fmtDur(dur)}
        </span>
        {variants && <Tip label={`${c.variant.candidates.length} aday çekim`}><span className="flex h-6 items-center gap-0.5 rounded-md bg-black/50 px-1.5 text-[11px] text-white/85 backdrop-blur-sm"><Layers size={11} />{c.variant.candidates.length}</span></Tip>}
      </div>
      ...
      <button
        ...
        aria-label={c.enabled ? `Sahne ${c.scene} klibini sil` : `Sahne ${c.scene} klibini geri al`}
        title={c.enabled ? 'Klibi sil — kurguya girmez (geri alınabilir)' : 'Geri al — klibi kurguya döndür'}
        className={cn('absolute bottom-2 right-2 z-10 flex h-7 w-7 items-center justify-center rounded-full opacity-0 shadow-[0_2px_8px_rgba(0,0,0,.6)] backdrop-blur-sm transition-all duration-150 group-hover:opacity-100',
      ...
      {width >= 232 && <span className="absolute left-2 top-9 flex items-center gap-1 rounded bg-black/45 px-1.5 py-0.5 text-[10.5px] text-white/85 backdrop-blur-sm"><span className="h-1.5 w-1.5 rounded-full" style={{ background: scaleColor(c.meta.scale) }} />{scaleLabel(c.meta.scale)}</span>}
...
function TransitionBubble({ clip: c, selected, onClick }: { clip: Clip; selected: boolean; onClick: () => void }) {
  ...
  const isCut = t === 'cut'
  const nextLabel = TRANSITION[NEXT_TYPE[t]].label
  // tıkla → seç + döngü: cut → fade → black → cut
  const cycle = () => { onClick(); setType(c.scene, NEXT_TYPE[t]) }
  return (
    <Tip label={<span>{meta.label}{!isCut && ` · ${fmtDur(c.transition_in!.dur)}`} — tıkla: <b>{nextLabel}</b>'a çevir</span>}>
      <button onClick={cycle} className={cn('group relative flex shrink-0 items-center justify-center self-center', isCut ? 'w-9' : 'w-16')}>
        ...
        {isCut ? (
          <span className={cn('relative z-10 flex h-7 w-7 items-center justify-center rounded-full transition-transform group-hover:scale-110', selected && 'ring-2')}
            style={{ background: `color-mix(in srgb, ${meta.color} 30%, #0b0e14)`, boxShadow: `0 0 0 1.5px color-mix(in srgb, ${meta.color} 60%, transparent)`, ['--tw-ring-color' as string]: meta.color }}>
          ...
          <span className={cn('relative z-10 flex flex-col items-center gap-0.5 rounded-xl px-2 py-1.5 transition-transform group-hover:scale-110', selected && 'ring-2')}
            style={{ background: `color-mix(in srgb, ${meta.color} 26%, #0b0e14)`,
            ...
            <span className="text-[9px] font-semibold tabular leading-none" style={{ color: meta.color }}>{c.transition_in!.dur.toFixed(1)}</span>
          </span>
        )}
      </button>
    </Tip>
