...
const SLOTS: Slot[] = [
  ...
  { key: 'vprompt', title: 'Video Prompt Belgesi', hint: 'İsteğe bağlı · kamera-hareket sinyali', icon: <Film size={20} />, required: false, sample: '55-rainyboat_video.md' },
]

const ONBOARD = [
  { n: 1, icon: <Upload size={16} />, title: 'Ver', desc: 'Görsel prompt belgesi + video klasörünü seç. Video prompt belgesi opsiyonel.' },
  { n: 2, icon: <SlidersHorizontal size={16} />, title: 'İncele', desc: 'AutoReji ~160 klibi analiz eder; geçişleri ve kırpmaları ayarlar. Film şeridinde gözden geçir, dilediğini değiştir.' },
...
const VAL_ROWS = [
  ...
  { warn: false, label: 'Yedek çekimler ayıklandı', detail: 'aynı sahnenin birden çok videosundan en iyisi (1080p) seçildi' },
  { warn: true, label: 'Çözünürlük: 159 × 1080p · 1 × 720p', detail: 'sahne 115 işaretlendi' },
    ...
    <div className="relative h-full overflow-y-auto">
      ...
      <div className="relative mx-auto flex min-h-full max-w-5xl flex-col px-8 py-12">
        <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.5, ease: EASE.outExpo }}>
          ...
          <h1 className="mt-2 text-display font-semibold leading-tight">Bir bölüm kuralım</h1>
            ...
            return (
              ...
              >
                <div className="flex w-full items-center justify-between">
                  ...
                  {s.required ? <Badge color="var(--color-amber-400)">zorunlu</Badge> : <Badge>isteğe bağlı</Badge>}
                </div>
                <div>
                  <div className="text-lead font-medium">{s.title}</div>
                  <div className="mt-0.5 text-body leading-snug text-fg-subtle">{s.hint}</div>
                </div>
                <div className={cn('mt-1 w-full truncate rounded-lg px-2.5 py-1.5 text-label tabular transition-colors',
        ...
        {ready ? (
          ...
          <motion.div initial={{ opacity: 0, y: 8 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.4, ease: EASE.outExpo }} className="relative mt-9 overflow-hidden rounded-2xl glass p-5">
            ...
            {realPicked ? (
              <div className="mt-4 flex items-start gap-2.5 text-body text-fg-muted">
                <CircleCheck size={15} className="mt-0.5 shrink-0 text-ok" />
                <span>Gerçek girdiler seçildi. <b className="text-fg">Eşleşme, çözünürlük ve varyantlar</b> analiz adımında her klipten çıkarılır her klip tek tek incelenerek — sonuç İnceleme'de görünür.</span>
              </div>
            ) : (
            ...
            <div className="mt-5 flex flex-col gap-1.5 hairline-t pt-4">
              <label className="text-caption font-medium uppercase tracking-[0.14em] text-fg-subtle">Bölüm adı</label>
              <input value={name} onChange={(e) => setName(e.target.value)}
                className="w-full max-w-md rounded-lg bg-ink-900/70 px-3 py-2 text-ui text-fg ring-hair outline-none focus:ring-1 focus:ring-amber-400/50 select-text" />
              <span className="text-caption text-fg-subtle">Prompt belgesinin adından türetildi · çıktı ve arşiv bu adı kullanır</span>
            </div>
          </motion.div>
        ) : (
          /* Boş durum — "nasıl çalışır" 3 adım + örnek dene */
          <div className="mt-9">
            <span className="text-caption font-medium uppercase tracking-[0.14em] text-fg-subtle">Nasıl çalışır</span>
            <div className="mt-4 grid grid-cols-1 gap-4 md:grid-cols-3">
              {ONBOARD.map((st, i) => (
                <motion.div key={st.n}
                  ...
                  <div className="text-lead font-medium">{st.title}</div>
                  <div className="text-body leading-snug text-fg-subtle">{st.desc}</div>
                </motion.div>
              ))}
            </div>
            {!tauriAvailable() && (
              <p className="mt-5 text-body text-fg-subtle">
  ...
  return (
    <div className="flex items-start gap-2.5" style={{ opacity: passed ? 1 : 0.4, filter: passed ? 'none' : 'blur(1.3px)', transition: 'opacity .3s ease, filter .3s ease' }}>
      ...
      <div className="min-w-0">
        <div className="text-body leading-snug text-fg">{label}</div>
        <div className="text-label text-fg-subtle">{detail}</div>
      </div>
    </div>
  )
}
