      ...
      <div className="relative mx-auto max-w-3xl px-8 py-12">
        ...
        {phase === 'ready' && (
          <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.4, ease: EASE.outExpo }}>
            ...
            <div className="mt-3 flex items-center gap-3 rounded-xl glass px-4 py-2.5 text-body text-fg-muted">
            ...
            <div className="mt-7">
              <RevealPanel title="Premiere'de ne kurulacak" defaultOpen={false}>
                <div className="grid grid-cols-1 gap-2.5 sm:grid-cols-2">
                  <Feature icon={<Volume2 size={16} />} title="Native stereo" desc="tek ses katmanı, sol-sağ gerçek stereo — ASMR kesintisiz" />
                  <Feature icon={<Layers size={16} />} title="Düzenlenebilir klipler + geçişler" desc="her şey elle ince ayarlanabilir" />
                  <Feature icon={<Ban size={16} />} title="Render YOK" desc="anında, kayıpsız zaman çizelgesi" />
            ...
            {setup.plugin === 'pending' && (
              <div className="mt-7 flex items-start gap-2.5 rounded-xl bg-amber-500/[0.08] px-3.5 py-2.5 ring-1 ring-amber-400/25">
                ...
                <span className="flex-1 text-body leading-snug text-fg-muted">
          ...
          <div className="pt-6">
            <div className="flex items-center gap-5">
              <ProgressRing value={prog} size={92} stroke={5}>
                <span className="text-title font-semibold tabular leading-none text-gold">{Math.round(prog * 100)}<span className="text-caption">%</span></span>
              </ProgressRing>
              <div>
                <h1 className="text-headline font-semibold">Hazırlanıyor…</h1>
                <p className="mt-1 text-body text-fg-subtle">Kurgu planı ve manifest hazırlanıyor · <b className="text-fg-muted">render yok</b></p>
              </div>
            </div>
        ...
        {phase === 'done' && (
          <motion.div initial={{ opacity: 0, scale: 0.97 }} animate={{ opacity: 1, scale: 1 }} transition={{ duration: 0.45, ease: EASE.outExpo }} className="pt-4">
            <div className="relative flex flex-col items-center text-center">
              ...
              <p className="mt-1.5 max-w-md text-ui text-fg-muted">Kurgu planın tamam. Şimdi manifest dosyasını kaydet ve MONTAJCI panelinde aç — Premiere zaman çizelgesini render almadan, native stereo kurar.</p>
            </div>
            ...
            <div className="mt-7 space-y-4 rounded-2xl glass p-5">
              {setup.plugin === 'pending' && (
                <button onClick={() => setScreen('setup')} className="flex w-full items-center gap-2.5 rounded-xl bg-amber-500/[0.08] px-3.5 py-2.5 text-left ring-1 ring-amber-400/25 transition-colors hover:bg-amber-500/[0.12]">
                  ...
                  <span className="flex-1 text-body leading-snug text-fg-muted">MONTAJCI paneli henüz bağlı değil — bu olmadan bölüm Premiere'de açılmaz. <b className="text-amber-300">Hazırlığı aç →</b></span>
                </button>
              )}
              ...
              <div className="hairline-t pt-4">
                <div className="mb-2.5 flex items-center gap-1.5 text-caption font-medium uppercase tracking-[0.14em] text-fg-subtle">
                  <Download size={12} className="text-amber-400" /> Manifest dosyası
                </div>
                {!saved ? (
                  <>
                    <p className="mb-3 text-body leading-snug text-fg-muted">Panelin okuyacağı <code className="rounded bg-ink-900/70 px-1.5 py-0.5 font-mono text-label text-amber-200/85 ring-hair">{name}_manifest.json</code> dosyasını kaydet — <b className="text-fg-muted">nereye kaydedeceğini sen seçersin</b>.</p>
                  ...
                  <div className="rounded-xl bg-ok/[0.06] p-3.5 ring-1 ring-ok/20">
                    <div className="flex items-center gap-2 text-body font-medium text-ok"><Check size={15} /> Manifest kaydedildi</div>
                    <SavedPathRow name={saved.name} path={saved.path} />
                    <button onClick={saveManifest} className="mt-2.5 inline-flex items-center gap-1.5 text-label text-fg-subtle transition-colors hover:text-fg"><Download size={12} /> Tekrar kaydet (başka yere)</button>
                  </div>
                )}
              </div>
            ...
            <div className="mt-7 flex flex-wrap justify-center gap-2.5">
              ...
              <Button variant="subtle" onClick={() => setScreen('archive')}><Library size={16} /> Arşivi aç</Button>
...
function Feature({ icon, title, desc }: { icon: ReactNode; title: string; desc: string }) {
  return (
    <div className="flex items-start gap-3 rounded-xl glass p-3.5">
      ...
      <div><div className="text-body font-medium">{title}</div><div className="text-label text-fg-subtle">{desc}</div></div>
    </div>
  )
}
...
function Recap({ stats, downgraded }: { stats: Stats; downgraded: number }) {
  const lines: { txt: ReactNode; warn?: boolean }[] = [
    { txt: <><b className="text-fg">Native stereo</b> · {stats.enabled} klip tek ses katmanında — sol-sağ gerçek stereo</> },
    ...
    { txt: <>Boşluklar kapatıldı · <b className="text-fg">render yok</b>, kayıpsız zaman çizelgesi</> },
  ...
  return (
    <div className="mt-2.5">
      <div className="flex items-center gap-1.5">
        <code className="min-w-0 flex-1 truncate rounded-md bg-ink-900/70 px-2 py-1.5 font-mono text-label text-amber-200/85 ring-hair select-text">{value}</code>
      ...
      {!path && (
        <p className="mt-1.5 text-caption leading-snug text-fg-subtle">Yeri sen seçtin. Tarayıcı güvenlik gereği tam klasör yolunu gösteremez — kurulu uygulamada (.app) burada tam yol görünür.</p>
      )}
    </div>
  )
}
