...
const STAGES: { label: string; sub: string; icon: ReactNode }[] = [
  ...
  { label: 'Sahne ↔ video eşleştiriliyor', sub: 'varyant + ffprobe sağlık', icon: <GitCompareArrows size={15} /> },
  { label: 'Prompt benzerliği', sub: 'sahne-içi konum değişimi', icon: <Spline size={15} /> },
  { label: 'Video analizi + kareler', sub: 'hareket · QC · orta kare', icon: <ImageDown size={15} /> },
  { label: 'Görsel-AI sahne sinyali', sub: 'enerji · rol · ritim (yerel)', icon: <ScanEye size={15} /> },
  { label: 'Geçiş kararları + manifest', sub: 'durum makinesi + ritim', icon: <Wand2 size={15} /> },
]
...
export function AnalysisScreen() {
  ...
  useEffect(() => {
    ...
    loadEpisode()
      ...
      .catch((e: unknown) => { if (aliveRef.current) { setErrMsg(e instanceof Error ? e.message : 'Bilinmeyen hata'); setPhase('error') } })
  ...
  return (
    <div className="relative flex h-full items-center justify-center overflow-hidden">
      ...
      <motion.div initial={{ opacity: 0, scale: 0.97 }} animate={{ opacity: 1, scale: 1 }} transition={{ duration: 0.5, ease: EXPO }}
        className="relative w-[520px] max-w-[92vw] overflow-hidden rounded-[1.7rem] glass p-8 shadow-[var(--shadow-pop)]">
        ...
        <AnimatePresence mode="wait">
          {phase === 'error' ? (
            ...
            <motion.div key="done" initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ duration: 0.3 }} className="flex flex-col items-center text-center">
              ...
              <motion.h2 initial={{ opacity: 0, y: 6 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.15 }} className="mt-5 text-[19px] font-semibold">Bölüm analizi tamamlandı 🎬</motion.h2>
              <motion.p initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ delay: 0.3 }} className="mt-1 flex items-center gap-1.5 text-[13px] text-fg-muted">
                Reji'ye geçiliyor
            ...
            <motion.div key="run" initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0, scale: 0.98 }} transition={{ duration: 0.2 }}>
              ...
              <div className="flex flex-col items-center">
                <ProgressRing value={stageFill} size={132} stroke={6}>
                  <span className="text-[30px] font-semibold tabular leading-none text-gold">{stagePct}<span className="text-[15px]">%</span></span>
                  <span className="mt-1 text-[10px] tabular text-fg-subtle">aşama {stageIdx + 1} / {N}</span>
                </ProgressRing>
                <h2 className="mt-5 text-[16px] font-semibold">{cur.label}</h2>
                <p className="text-[12px] text-fg-subtle">{cur.sub} · tamamen yerel, offline</p>
              </div>
                    ...
                    <div key={i} className="flex items-center gap-3" style={{ opacity: status === 'pending' ? 0.4 : 1, transition: 'opacity .35s ease' }}>
                      ...
                      <div className="min-w-0 flex-1">
                        <div className="flex items-center justify-between gap-2">
                          ...
                          <span className="shrink-0 text-[10.5px] tabular text-fg-subtle">{status === 'active' ? `${stagePct}%` : ''}</span>
                        </div>
