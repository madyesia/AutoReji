...
- **Süre/kırpma EVRİLDİ (v1.0.1, kullanıcı onayıyla):** "hedef-süre" modeli — tutulan süre **[4.30s SERT TABAN, ~7s]**; görsel-AI `linger`/enerji birincil kaldıraç; **AI-üretim baş(~0.5s)/son(~0.7s) artefaktı garanti kesilir**. (GEÇİŞ kararları yine donmuş.)
- **Görsel-AI rolü netleşti:** VLM (Qwen2.5-VL 7B, yerel/Ollama) yalnız **kurgu sinyali** (energy/role/linger) — **hata/QC denetimi KALDIRILDI** (7B anime/silüette güvenilmez). Teknik QC ayrı (crop + heuristik).
- **Açılış/kapanış imzası VARSAYILAN KAPALI** (kanal sahibi tercihi; intro/outro fade = 0).
...
## 🔮 VİZYON / GELECEK (NOT — UYGULANMADI; bu Blueprint mevcut ürün içindir)
Kullanıcının daha büyük planı AYRI belgelerde (mevcut AutoReji'nin offline/render-yok ilkelerini DEĞİŞTİRMEZ):
- **`docs/YOL_HARITASI_VE_VIZYON.md`** — ticarileşme (kapalı beta → portföy → resmî satış) + **"Genel Reji SaaS"** vizyonu: ASMR'dan çıkıp **her tür kurgu** · **online abonelik/üyelik** (mevcut offline AutoReji'den AYRI 2. ürün) · içerik-odaklı stil tespiti (dinamik/soft/normal) · **20-30 dil** (tüm arayüz) · **otomatik altyazı + stiller** (yerel Whisper) · video format (dikey/yatay/kare + auto-reframe) · **gömülü ffmpeg render motoru** (çözünürlük/kodek) · **çok-NLE çıktı** (Premiere/DaVinci/FinalCut/CapCut — OTIO/FCPXML). Mimari öneri: yerel-işleme + bulut-hesap. + ek özellik fikirleri (beat-sync müzik, sessizlik/dolgu-kelime kesme, highlight tespiti, marka kiti, doğrudan yayınlama…).
- **`docs/GUNCELLEME_MIMARISI.md`** — oto-güncelleme + temalı açılış (splash) ekranı + merkezi sürüm feed.
...
- De-risk sonucunu buraya yaz: `VideoTransition` (Cross Dissolve / Dip to Black) tam erişim yolu, `AddTransitionOptions` süre alanı, kırpma için kesin Action zinciri.
...
🔧 **CLAUDE CODE — FAZ 2 KALİBRASYONU (DONDURULDU 2026-06-29, v1.7.0):**
- **Kanonik kaynak = `config/config.toml`** (yukarıdaki varsayılanlar başlangıç niyetiydi; **gerçek dondurulmuş değerler** config.toml'da). `decide.py`/`build_manifest.py` `Config.from_toml()` ile okur; eksik alan kod varsayılanına düşer.
- **Karar:** `hysteresis=3`, **`sim_block_thresh=0.18`** (Jaccard; başlangıçtaki `0.40/0.55` kosinüs varsayımı yerine — gerçek Jaccard ölçeği düşük), `min_cuts_between_fades=2`, `no_back_to_back_black=true`.
- **Geçiş:** fade 1.0–2.0s / black 1.5–2.5s, **hepsi kare-hizalı**; sarsıcı-cut yumuşatma fade'i 0.5–0.9s.
- **Kırpma/handle:** `trim_base=1.5`, `trim_jit=1.5`, **`trim_min=1.0`** (her klip her yandan ≥1s garanti handle → fade DAİMA ortalanır), `trim_max=2.5`, `transition_edge_extra=0.5`.
- **Derin analiz:** `jarring_contrast=45`, `motion_calm=1.5`, `motion_busy=4.0`, `motion_trim_k=0.07`.
- **Tempo modülasyonu** (trim.py'de gömülü, config.toml `[trim.modulation]`'da belgeli): iç/uyku −0.35, dış +0.25, kuruluş/drone −0.35, yakın +0.30, energy (e−3)·0.13, role ±0.16/0.10, linger −0.16.
- **Sonuç (Bölüm 2, 160 klip):** CUT 117 / FADE 40 / BLACK 2 (~%74/%25/%1 cozy), süre 3.5–5.9s, ~13.9 dk — **kullanıcı onayladı**.
- **20 bölüm doğrulaması (3200 sahne, `örnek_image_prompts/`):** her bölüm tam 160 sahne; 48 karakter kodu 0 yanlış eşleşme; `sim_block_thresh=0.18` 20-bölüm medyanının (0.22) hemen altında → Bölüm 2'ye özel değil; **tek genel algoritma yeterli** (§6 🔧 — tip-özel eşik gerekmedi).
- **Temel/Gelişmiş ayrımı:** Temel = mod + focus_review + arşiv yolu; Gelişmiş (Yönetmen) = tüm eşikler/aralıklar/rafineler. Config şeması `[meta]` ile sürümlü; her manifest `config_hash` taşır.
- **Açık:** video/VLM sinyalleri yalnız Bölüm 2'de kalibre; renk tespiti dar (%30 unknown, dış bölümlerde). Gerçek farklı bölüm gelince izle.
